...
The dashboard shows the ARMS RACE:
  ...
  - CIS control mapping + CISO posture report
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  button{background:var(--panel);color:var(--ink);border:1px solid var(--line);
         padding:6px 14px;border-radius:8px;cursor:pointer;font:inherit;font-size:12px}
  ...
  .progress-bar{height:100%;background:var(--blue);width:0;transition:width .1s}
  .round-label{font-size:12px;color:var(--muted);min-width:80px}
</style>
</head>
<body>
<header>
...
<div class="grid">
  ...
  <div class="card">
    ...
    <canvas id="c4" style="height:180px"></canvas>
  </div>
...
function lineChart(canvas, series, opts){
  ...
  if(DATA.shock && DATA.shock.round>0 && !opts.skipShock){
...
const covItems=defIds.map(d=>({label:DATA.defenses[d].slice(0,18), v:DATA.coverage_final[d]}));
const techIds=Object.keys(DATA.threat_final);
const thrItems=techIds.map(t=>({label:t.slice(0,18), v:DATA.threat_final[t]}));
(function(){
  ...
  covItems.forEach((d,i)=>{const y=18+i*(half-18)/covItems.length; const bh=(half-18)/covItems.length*0.7;
    ...
    ctx.fillStyle=C.blue; ctx.fillRect(130,y,(w-160)*(d.v/cMax),bh);});
  ...
  thrItems.forEach((d,i)=>{const y=t0+16+i*(half-16)/thrItems.length; const bh=(half-16)/thrItems.length*0.7;
    ctx.fillStyle=C.muted; ctx.fillText(d.label,4,y+bh);
    ctx.fillStyle=C.red; ctx.fillRect(130,y,(w-160)*(d.v/tMax),bh);});
})();
...
if(sh.round){
  shEl.innerHTML =
    ...
    '<span style="color:var(--green)">The defender adapts to the unknown.</span>';
} else {
...
function drawReplayFrame(round){
  const dpr = window.devicePixelRatio||1;
  const w = c4.clientWidth, h = c4.clientHeight;
  c4.width=w*dpr; c4.height=h*dpr;
  const ctx = c4.getContext('2d'); ctx.scale(dpr,dpr);
  ctx.clearRect(0,0,w,h);
  
  const pad={l:38,r:10,t:20,b:30};
  const shown = H.slice(0, round+1);
  const maxR = Math.max(...shown.map(h=>h.residual_risk), ...shown.map(h=>h.red_win_rate)) * 1.1 || 1;
  ...
  // labels
  ctx.fillStyle=C.muted; ctx.font='10px monospace';
  ctx.fillText('RED win', pad.l+4, pad.t+10);
  ctx.fillStyle=C.muted;
  ctx.fillText('risk', pad.l+4, pad.t+24);
  
  const X = i => pad.l + (w-pad.l-pad.r)*(i/N);
  const Y = v => h-pad.b - (h-pad.t-pad.b)*(v/maxR);
  
  // RED win rate line
  ctx.strokeStyle=C.red; ctx.lineWidth=2; ctx.beginPath();
  shown.forEach((h,i)=>{const x=X(i),y=Y(h.red_win_rate); i?ctx.lineTo(x,y):ctx.moveTo(x,y);});
  ctx.stroke();
  
  // Residual risk line
  ctx.strokeStyle=C.green; ctx.lineWidth=2; ctx.beginPath();
  shown.forEach((h,i)=>{const x=X(i),y=Y(h.residual_risk); i?ctx.lineTo(x,y):ctx.moveTo(x,y);});
  ctx.stroke();
  
  // Current round marker
  const curX = X(round);
  ctx.strokeStyle='rgba(255,255,255,.5)'; ctx.setLineDash([3,3]);
  ...
  ctx.fillText('Attack: '+cur.attack, pad.l+4, h-pad.b-6);
  ctx.fillStyle=cur.success?C.red:C.muted;
  ctx.fillText(cur.success?'HIT':'MISS', pad.l+180, h-pad.b-6);
  
  // Update controls
  roundLabel.textContent = 'Round '+round;
  progressBar.style.width = ((round+1)/N*100)+'%';
}

function play(){
  if(animRound >= N-1) animRound = 0;
  btnPlay.classList.add('active');
  btnPause.classList.remove('active');
  function step(){
    drawReplayFrame(animRound);
    animRound++;
    if(animRound >= N){
      pause();
      return;
